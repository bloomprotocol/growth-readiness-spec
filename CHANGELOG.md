...
- Approval-gated runtimes such as Hermes, Claude Code, OpenClaw, and Codex
  should show a task-level preflight before terminal/network/filesystem prompts,
  prefer native HTTP tools over shell `curl`, and batch safe skill-install file
  operations into one explained approval.
